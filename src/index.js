...
const App = {
  ...
  registerEstate: async function(_landOwner , _landOwerName, _location,  _cost , _lengthLand, _widthLand){
    const { registerEstate } = this.meta.methods;
    await registerEstate(_landOwner , _landOwerName, _location,  _cost ,_lengthLand, _widthLand).send({
      from: this.account
    });
  },
