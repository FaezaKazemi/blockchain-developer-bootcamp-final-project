...
contract RealEstateContract{
 //Declare the struct
// Estate struct keeps the information of the land
 ...
 using SafeMath for uint;
  //Mapping
//link addresses their esate structs through this mapping
...
///@notice registerEstate function uses details of struct to register the land
//push the estate to the government’s estates mapping and emit the register event.
...
///@notice transferEstate function is used for transfering the estate
function transferEstate(address _buyer, address _seller, uint _ID , string memory sellerName ) public isOwner returns (bool){
    ...
    ///@notice getEstate function is used to view the estate details of someone’s account
function getEstate(address _estateOwner) public view returns (uint ,string memory, string memory, uint, uint ,uint, uint)
    {
        Estate memory e = estates [_estateOwner];
        return(e.id , e.landOwnerName, e.location, e.cost, e.lengthLand, e.widthLand, e.area);
}
///@notice getNumberOfEstates function is used to retern the number of estates the account has
function getNumberOfEstates() public  view returns (uint)
    {
        return totalEstateCounter;
}
///@notice findEstateId function is used to retern the id of the estate by using its address
